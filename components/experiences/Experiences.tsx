...
const Experiences = ({ experienceData, educationData }: Props) => {
    ...
    const [show, setShow] = useState(experienceData.length > 0 ? "Experience" : "Education")
    ...
    const tabs = []
    if (experienceData.length > 0) tabs.push('Experience')
    if (educationData.length > 0) tabs.push('Education')

    return (
        <SectionWrapper id="experience" className="min-h-screen">
            <h2 className="text-4xl text-center">{tabs.length === 1 ? tabs[0] : "Education"}</h2>

            {tabs.length > 1 && <div className="w-fit mx-auto mt-6 p-2 bg-white dark:bg-grey-800 rounded-md flex gap-2 items-center">
                {tabs.map((e, i) => (
